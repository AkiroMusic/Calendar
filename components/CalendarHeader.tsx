...
const CalendarHeaderComponent: React.FC<CalendarHeaderProps> = ({
  ...
  return (
    <div className="bg-transparent pt-3 px-6 pb-2 border-b border-surface-border relative">
      <div className="flex justify-between items-start relative">
        ...
        <div className="relative">
            ...
            >
                ...
                <div className="flex flex-col justify-end pb-1">
                    ...
                    <div className="text-text-secondary font-sans text-xs font-semibold flex items-center gap-2 select-none">
                ...
                <div className="flex gap-1 pb-1 ml-2" onClick={(e) => e.stopPropagation()}>
                    <button onClick={onPrevMonth} className="p-1 hover:bg-surface-hover rounded-full text-text-secondary transition-colors" title="Previous month">
                    <ArrowLeft size={16} />
                    </button>
                    <button onClick={onNextMonth} className="p-1 hover:bg-surface-hover rounded-full text-text-secondary transition-colors" title="Next month">
                    <ArrowRight size={16} />
                    </button>
                </div>
            </div>
            ...
            {isSelectorOpen && (
                <>
                    ...
                    <div className="absolute top-full left-0 mt-4 z-40 bg-surface/95 backdrop-blur-sm border border-surface-border shadow-2xl rounded-xl p-4 w-[320px] animate-in fade-in zoom-in-95 slide-in-from-top-2 origin-top-left">
                        {/* Year Navigation */}
                        <div className="flex justify-between items-center mb-4 border-b border-surface-border pb-2">
                            <button onClick={prevYear} className="p-1 hover:bg-surface-hover rounded-lg text-text-secondary transition-colors" title="Previous year">
                                <ChevronLeft size={20} />
                            </button>
                            <span className="font-serif font-bold text-xl text-ink-black">{selectorYear}</span>
                            <button onClick={nextYear} className="p-1 hover:bg-surface-hover rounded-lg text-text-secondary transition-colors" title="Next year">
                                    ...
                                    <button
                                        ...
                                        className={`
                                            ...
                                            ${isSelected 
                                                ...
                                                : isRealCurrent
                                                    ? 'bg-red-950 text-ink-red font-semibold ring-1 ring-inset ring-red-900'
                                                    : 'text-text-secondary hover:bg-surface-hover hover:text-ink-blue'}
                                        `}
                                    >
                                        {getMonthName(i)}
        ...
        <div className="flex-1 mx-8 mt-1">
            <div className="flex justify-between items-center mb-1">
                <span className="text-[9px] font-bold tracking-widest text-text-secondary uppercase select-none">{t('monthlyPlan')}</span>
            </div>
            <div className="flex flex-col gap-1">
                {monthlyPlan.map((plan, index) => (
                    <div key={index} className="flex items-center gap-2 border-b border-surface-border/60 pb-0.5">
                        ...
                        <input 
                            ...
                            className="w-full bg-transparent outline-none text-xs font-hand text-ink-black placeholder-stone-600"
